...
/**
 * ネゴシエーション結果表示コンポーネント
 * AIによる動的役割決定の結果を段階的に可視化
 */
export function NegotiationResult({ result }: NegotiationResultProps) {
  // ネゴシエーション中（transcript があるが success がまだ未定義）
  const isNegotiating = result?.isNegotiating || (result?.transcript && result.transcript.length > 0 && result.success === undefined);
  
  return (
    <Card glow="cyan" className="w-full">
      <CardContent className="p-4">
        {result && (result.success !== undefined || isNegotiating) ? (
          // ネゴシエーション進行中または完了
          <div className="space-y-3">
            ...
            {result.success === true ? (
              ...
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-cyan-400 font-bold">Negotiating...</h3>
              </div>
            )}

            {/* AI決定の役割（結果が確定したら表示） */}
            {result.success === true && result.buyer && result.seller && (
            ...
            {/* 合意価格（結果が確定したら表示） */}
            {result.success === true && result.agreedPrice && (
              ...
              <div className="text-xs max-h-40 overflow-y-auto space-y-1">
                {result.transcript.map((line, i) => {
                  ...
                  if (txMatch) {
                    ...
                    // ネットワークを判定（Sepoliaかどうか）
                    const isSepolia = line.includes('Sepolia') || line.includes('EIP-7702');
                    ...
                    return (
                      <p key={i} className="text-green-400 animate-fade-in">
                        ...
                        {isSepolia && <span className="text-xs text-purple-400 ml-2">(Sepolia)</span>}
                      </p>
                    );
                  }
